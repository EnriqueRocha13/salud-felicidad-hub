...
export default function AdminLayout() {
  ...
  const navItems = [
    { to: "/admin", label: t("admin.products"), icon: Package },
    { to: "/admin/bundles", label: t("admin.bundles"), icon: Layers },
    { to: "/admin/users", label: t("admin.users"), icon: Users },
    { to: "/admin/orders", label: t("admin.orders"), icon: ShoppingBag },
    { to: "/admin/chat", label: t("admin.support"), icon: MessageSquare },
  ];
        ...
        <div className="border-t pt-4 space-y-2 shrink-0">
          ...
          <Link to="/">
            <Button variant="ghost" className="w-full justify-start gap-2">
              <Home className="h-4 w-4" /> {t("admin.public_site")}
            </Button>
          </Link>
